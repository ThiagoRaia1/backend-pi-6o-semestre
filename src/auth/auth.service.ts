...
export class AuthService {
  ...
  async validateUser(email: string, password: string) {
    ...
    if (user && bcrypt.compareSync(password, user.senha) && user.isAtivo) {
  ...
  async login(user: { id: number; nome: string }) {
    ...
    // Retornamos o token e o nome do usuário separadamente, pois apenas o token será salvo no AsyncStorage enquanto nome permanecerá apenas no contexto da aplicação
    return {
